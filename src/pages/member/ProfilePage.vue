...
// Ministry Involvement
async function fetchMinistryInvolvement() {
    if (!auth.session?.user) return
    const uid = auth.session.user.id

    const [invRes, bodRes] = await Promise.all([
        supabase
            .from('tbl_ministry_involvements')
            .select('id, ministry_id, ministry_type, ministry_role_title, member_type, skills, date_started, is_active, ministry:lib_ministries!tbl_ministry_involvements_ministry_id_fkey(ministry_type)')
            .eq('user_id', uid)
            .order('date_started', { ascending: false }),
        supabase
            .from('tbl_bod_members')
            .select('id, date_started, is_active')
            .eq('user_id', uid)
            .eq('is_active', 'Y')
            .maybeSingle(),
    ])

    ministryInvolvements.value = (invRes.data ?? []) as MinistryInvolvement[]
    bodMembership.value = bodRes.data ?? null
}
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <template v-else>
            ...
            <div class="relative h-48 sm:h-56 rounded-xl overflow-hidden bg-gradient-to-br from-navy via-navy-700 to-navy-600">
                ...
                <label v-if="auth.isApproved" class="absolute bottom-3 right-3 inline-flex items-center gap-1.5 px-3 py-1.5 bg-black/30 hover:bg-black/50 text-white text-xs font-medium rounded-lg backdrop-blur-sm cursor-pointer transition-colors">
            ...
            <div class="relative px-4 sm:px-6">
                ...
                <div class="-mt-14 sm:-mt-16 flex items-end justify-between mb-4">
                    <div class="relative group/avatar shrink-0">
                        ...
                        <label v-if="auth.isApproved" class="absolute inset-0 rounded-full flex items-center justify-center bg-black/40 text-white opacity-0 group-hover/avatar:opacity-100 cursor-pointer transition-all">
            ...
            <!-- Dedicated Edit Form (full width, replaces all content) -->
            <div v-if="editMode" class="bg-white rounded-lg border border-gray-200 mb-6">
                <!-- Header with Save + Cancel -->
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h2 class="font-heading font-semibold text-navy text-lg">Edit Profile</h2>
                    <div class="hidden sm:flex items-center gap-2">
                        <button @click="editMode = false; populateForm()" class="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            Cancel
                        </button>
                        <button @click="handleSave" :disabled="saving" class="px-5 py-2 bg-navy text-white text-sm font-semibold rounded-lg hover:bg-navy-700 disabled:opacity-50 transition-colors">
                ...
                <form @submit.prevent="handleSave" class="p-6">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="form.first_name" type="text" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/30 focus:border-navy" />
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Last Name</label>
                                <input v-model="form.last_name" type="text" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/30 focus:border-navy" />
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Middle Name</label>
                                <input v-model="form.middle_name" type="text" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/30 focus:border-navy" />
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Ext (Jr, Sr, III)</label>
                                <input v-model="form.ext_name" type="text" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/30 focus:border-navy" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input :value="form.email" type="email" disabled class="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm bg-gray-50 text-gray-500" />
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Phone Number</label>
                                <input v-model="form.phone_number" type="tel" placeholder="e.g. +63 912 345 6789" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/30 focus:border-navy" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                        <h3 class="text-sm font-semibold text-gray-900 uppercase tracking-wider mb-4">Other Details</h3>
                        <div class="grid sm:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Birth Date</label>
                                <input v-model="form.birth_date" type="date" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/30 focus:border-navy" />
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-500 mb-1">Satellite Church</label>
                                <select v-model="form.satellite_church_id" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-navy/30 focus:border-navy">
                    ...
                    <div class="sm:hidden space-y-2">
                        ...
                        <button type="button" @click="editMode = false; populateForm()" class="w-full py-2.5 text-sm font-medium text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            Cancel
                        </button>
                    </div>
                </form>
            ...
            <!-- Profile Content (hidden during edit) -->
            ...
            <div class="grid lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <!-- Leadership Card -->
                    <div v-if="(member.network.myRole && member.network.myRole !== 'Member') || bodMembership" class="bg-white rounded-lg border border-gray-200 p-5">
                        <h2 class="font-heading font-semibold text-navy text-base mb-4">Leadership</h2>
                        <div class="space-y-3">
                            <!-- Leadership Role (Pastor / Network Leader / L-Path Leader) -->
                            <div v-if="member.network.myRole && member.network.myRole !== 'Member'" class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-[10px] text-gray-400 uppercase tracking-wider leading-none mb-0.5">Leader</p>
                                    <p class="text-sm font-medium text-gray-900">{{ member.network.myRole }}</p>
                                </div>
                            </div>

                            <!-- BOD Membership -->
                            <div v-if="bodMembership" class="flex items-start gap-3">
                                <div class="w-9 h-9 rounded-full bg-violet-50 flex items-center justify-center shrink-0">
                                ...
                                <div>
                                    <p class="text-[10px] text-gray-400 uppercase tracking-wider leading-none mb-0.5">Board of Directors</p>
                                    <p class="text-sm font-medium text-gray-900">BOD Member</p>
                                    <p v-if="bodMembership.date_started" class="text-xs text-gray-400">Since {{ formatDate(bodMembership.date_started) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Ministry Card -->
                    <div v-if="ministryInvolvements.length" class="bg-white rounded-lg border border-gray-200 p-5">
                        <h2 class="font-heading font-semibold text-navy text-base mb-4">Ministry</h2>
                        <div class="space-y-3">
                            <div v-for="inv in ministryInvolvements" :key="'inv-' + inv.id" class="flex items-start gap-3">
                                <div class="w-9 h-9 rounded-full flex items-center justify-center shrink-0"
                                    :class="inv.member_type === 'head' ? 'bg-amber-50' : 'bg-blue-50'"
                                >
                                    <svg v-if="inv.member_type === 'head'" class="w-4 h-4 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="min-w-0">
                                    <p class="text-[10px] text-gray-400 uppercase tracking-wider leading-none mb-0.5">
                                        {{ inv.member_type === 'head' ? 'Ministry Head' : 'Ministry Servant' }}
                                    </p>
                                    <p class="text-sm font-medium text-gray-900">{{ inv.ministry?.ministry_type ?? inv.ministry_type ?? 'Ministry' }}</p>
                                    <p v-if="inv.ministry_role_title" class="text-xs text-gray-500">{{ inv.ministry_role_title }}</p>
                                    <div v-if="Array.isArray(inv.skills) && inv.skills.length" class="flex flex-wrap gap-1 mt-1">
                                        <span v-for="(s, si) in inv.skills" :key="si" class="inline-block px-1.5 py-0.5 bg-blue-50 text-blue-600 text-[10px] rounded-full">{{ s }}</span>
                                    </div>
                                    <span class="inline-flex mt-1 px-1.5 py-0.5 rounded text-[10px] font-medium" :class="inv.is_active === 'Y' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'">
                                        {{ inv.is_active === 'Y' ? 'Active' : 'Inactive' }}
                                    </span>
                                </div>
                            </div>
                        </div>
