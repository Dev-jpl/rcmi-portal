...
function getReaction(devId: string, type: string): ReactionCount {
  ...
  return (
    devReactions?.find((r) => r.type === type) ?? {
      devotional_id: devId,
      type,
      count: 0,
      user_reacted: false,
...
function handleCommentInput(id: string, e: Event) {
  const el = e.target as HTMLInputElement
  setCommentInput(id, el.value)
}
    ...
    <div v-else class="space-y-4">
      ...
      >
        <div class="flex items-center gap-3 mb-2">
          <div
            class="w-8 h-8 rounded-full overflow-hidden shrink-0 bg-navy/10 flex items-center justify-center text-navy text-xs font-bold"
          >
            <img
              v-if="getAvatar(item.user_id)"
              :src="getAvatar(item.user_id) || ''"
              class="w-full h-full object-cover"
            />
            <span v-else>{{ item.author_name?.[0] ?? '?' }}</span>
          </div>
          ...
          <div v-if="item.user_id === auth.session?.user?.id" class="relative">
            ...
            <Transition name="dropdown">
              <div
                ...
                class="absolute right-0 top-8 w-36 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20"
              >
                <button
                  class="w-full flex items-center gap-2 px-3 py-2 text-xs text-gray-700 hover:bg-gray-50 transition-colors"
                  @click="startEditDevotional(item); closeMenus()"
                >
                  <svg
                ...
                <button
                  ...
                  @click="showDeleteConfirm = { type: 'devotional', id: item.id }; closeMenus()"
                >
        ...
        <div v-if="editingDevotional?.id === item.id" class="mb-3">
          ...
          <div class="flex justify-end gap-2 mt-2">
            <button
              ...
              @click="cancelEditDevotional"
            >
              Cancel
            </button>
            <button
              :disabled="editSaving || !editDevotionalContent.trim()"
              class="px-3 py-1.5 bg-navy text-white text-xs font-medium rounded-lg hover:bg-navy-800 disabled:opacity-50 transition-colors"
              @click="saveEditDevotional"
            >
        ...
        <div class="flex items-center gap-2 pt-2 border-t border-gray-100">
          <button
            ...
            :class="
              getReaction(item.id, rt.type).user_reacted
          ...
          >
            <span>{{ rt.emoji }}</span>
            <span v-if="getReaction(item.id, rt.type).count > 0">{{
              getReaction(item.id, rt.type).count
            }}</span>
        ...
        <div v-if="expandedComments.has(item.id)" class="mt-3 pt-3 border-t border-gray-100">
          ...
          <div v-if="getComments(item.id).length" class="space-y-2.5 mb-3">
            <div v-for="c in getComments(item.id)" :key="c.id" class="flex gap-2.5">
              <div
                class="w-6 h-6 rounded-full overflow-hidden shrink-0 mt-0.5 bg-gray-100 flex items-center justify-center text-[10px] font-bold text-gray-500"
              >
                <img
                  v-if="getAvatar(c.user_id)"
                  :src="getAvatar(c.user_id) || ''"
                  class="w-full h-full object-cover"
                />
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="editingComment?.id === c.id" class="bg-gray-50 rounded-lg px-3 py-2">
                  <input
                    ...
                    type="text"
                    class="w-full px-2 py-1 border border-gray-200 rounded text-xs focus:outline-none focus:ring-2 focus:ring-navy/20"
                  ...
                  <div class="flex justify-end gap-2 mt-1.5">
                    ...
                    <button
                      ...
                      class="text-[10px] text-navy font-medium hover:underline"
                      @click="saveEditComment"
                    >
                      {{ editCommentSaving ? '...' : 'Save' }}
                    </button>
                  </div>
                </div>
                <!-- Normal display -->
                <template v-else>
                  ...
                  <div class="flex items-center gap-2 mt-0.5 ml-1">
                    ...
                    <div v-if="c.user_id === auth.session?.user?.id" class="relative">
                      ...
                      <Transition name="dropdown">
                        <div
                          v-if="openCommentMenu === c.id"
                          class="absolute left-0 top-4 w-32 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20"
                        >
                          <button
                            class="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] text-gray-700 hover:bg-gray-50"
                            @click="startEditComment(c); closeMenus()"
                          >
                            Edit
                          </button>
                          <button
                            class="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] text-red-600 hover:bg-red-50"
                            @click="showDeleteConfirm = { type: 'comment', id: c.id, parentId: item.id }; closeMenus()"
                          >
                            Delete
                          </button>
                        </div>
          ...
          <div v-if="auth.isApproved" class="flex gap-2">
            <div
              class="w-6 h-6 rounded-full overflow-hidden shrink-0 mt-1 bg-navy/10 flex items-center justify-center text-[10px] font-bold text-navy"
            >
              <img
                v-if="auth.profile?.profile_photo_url"
                :src="auth.profile.profile_photo_url"
                class="w-full h-full object-cover"
              />
              <span v-else>{{
            ...
            <div class="flex-1 flex gap-2">
              <input
                ...
                @input="(e) => handleCommentInput(item.id, e)"
      ...
      <Transition name="fade">
        ...
        >
          ...
          <div class="relative bg-white rounded-lg shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <h3 class="font-heading font-semibold text-gray-900 text-lg mb-2">
              Delete {{ showDeleteConfirm.type === 'devotional' ? 'Post' : 'Comment' }}?
            </h3>
