...
async function fetchPrograms() {
  const { data } = await supabase.from('lib_programs').select('*').order('type')
...
async function handleSaveProgram() {
  ...
  if (editingProgram.value) {
    ...
    const { error } = await supabase.from('lib_programs').insert(payload)
    if (error) {
      message.value = { type: 'error', text: error.message }
    } else {
...
function openDeleteEnrollment(e: Enrollment) {
  deleteTarget.value = {
    type: 'enrollment',
    id: e.id,
    label: `${getMemberName(e.user_id)} from ${getProgramName(e.program_id)}`,
  }
  showDeleteModal.value = true
}
...
async function handleSaveEnrollment() {
  ...
  if (editingEnrollment.value) {
    ...
    const { error } = await supabase.from('tbl_program_involvements').insert(payload)
    if (error) {
      message.value = { type: 'error', text: error.message }
    } else {
  ...
  <div>
    ...
    <div class="mb-4">
      <input
        ...
        type="text"
        :placeholder="
          activeTab === 'library' ? 'Search programs...' : 'Search by member or program...'
        "
    ...
    <template v-else-if="activeTab === 'library'">
      <div v-if="!filteredPrograms.length" class="text-center py-12 text-gray-400">
        No programs found.
      </div>
              ...
              <tr v-for="p in filteredPrograms" :key="p.id" class="hover:bg-gray-50/50">
                <td class="px-4 py-3 font-medium text-gray-900">
                  <button
                    ...
                    @click="router.push({ name: 'admin-program-detail', params: { id: p.id } })"
                  >
                    {{ p.type }}
                  </button>
                </td>
                ...
                <td class="px-4 py-3">
                  <span
                    class="text-xs px-2 py-0.5 rounded-full font-medium bg-blue-50 text-blue-700"
                  >
                ...
                <td class="px-4 py-3 text-right">
                  <div class="flex items-center justify-end gap-2">
                    <button class="text-xs text-navy hover:underline" @click="openEditProgram(p)">
                      Edit
                    </button>
                    <button
    ...
    <!-- Enrollments Tab -->
    <template v-else>
      <div v-if="!filteredEnrollments.length" class="text-center py-12 text-gray-400">
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="e in filteredEnrollments" :key="e.id" class="hover:bg-gray-50/50">
                ...
                <td class="px-4 py-3 text-gray-500 hidden sm:table-cell">
                  {{ formatDate(e.date_started) }}
                </td>
                <td class="px-4 py-3 text-gray-500 hidden md:table-cell">
                  {{ formatDate(e.date_ended) }}
                </td>
                <td class="px-4 py-3">
                  <span class="text-xs font-medium text-gray-600">{{
                    getDuration(e.date_started, e.date_ended)
                  }}</span>
                </td>
                <td class="px-4 py-3">
                  <span
