...
const removeTarget = ref<NetworkLeader | null>(null)
...
const availableUsers = computed(() => {
  ...
  const existingIds = new Set(
    leaders.value
      .filter((m) => m.is_active === 'Y' && m.id !== editingId.value)
      .map((m) => m.user_id),
  )
  const approved = (admin.members as MemberProfile[]).filter(
    (m: MemberProfile) => m.status === 'approved' && !existingIds.has(m.user_id),
  )
...
async function handleSave() {
  ...
  if (editingId.value) {
    ...
    message.value = error
      ...
      : { type: 'success', text: 'Network leader updated.' }
  } else {
    const { error } = await supabase.from('tbl_network_leaders').insert(payload)
    message.value = error
      ? { type: 'error', text: error.message }
      : { type: 'success', text: 'Network leader added.' }
  }
...
function openRemove(m: NetworkLeader) {
...
async function handleRemove() {
  ...
  if (error) {
    message.value = { type: 'error', text: `Unable to remove network leader: ${error.message}` }
  } else {
    message.value = { type: 'success', text: 'Network leader assignment removed.' }
    await fetchLeaders()
  }
...
<template>
  <div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="m in filtered" :key="m.id" class="hover:bg-gray-50/50">
              ...
              <td class="px-4 py-3 text-gray-500 hidden lg:table-cell">
                {{ churchName(m.church_id) }}
              </td>
              <td class="px-4 py-3 text-gray-500 hidden md:table-cell">
                {{ formatDate(m.date_started) }}
              </td>
              <td class="px-4 py-3">
                <span
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
          <h3 class="text-lg font-heading font-bold text-gray-900 mb-2">Remove Network Leader</h3>
          <p class="text-sm text-gray-600">
            Remove <strong>{{ userName(removeTarget?.user) }}</strong> as a network leader? Their member account and profile will remain.
          </p>
          <div class="flex justify-end gap-3 mt-6">
