...
// UserOption no longer needed

const members = ref<BodMember[]>([])
...
onMounted(async () => {
  await Promise.all([fetchMembers(), admin.fetchMembers()])
  loading.value = false
})
...
async function fetchMembers() {
  const { data } = await supabase
    ...
    .select('*, user:tbl_users!tbl_bod_members_user_id_fkey(id, first_name, last_name, email)')
    .order('created_at', { ascending: false })
  members.value = (data as BodMember[]) ?? []
}

// No longer need separate fetchUsers as we use admin.members
...
const availableUsers = computed(() => {
  ...
  const approved = admin.members.filter(
    (m) => m.status === 'approved' && !existingIds.has(m.user_id),
  )
...
function openAdd() {
  ...
  form.value = { user_id: '', date_started: new Date().toISOString().split('T')[0], is_active: 'Y' }
  memberSearch.value = ''
  showMemberDropdown.value = false
...
async function handleSave() {
  ...
  if (editingId.value) {
    ...
    if (error) {
      ...
      message.value = { type: 'success', text: 'BOD member updated.' }
    }
  } else {
    const { error } = await supabase.from('tbl_bod_members').insert({
      user_id: form.value.user_id,
      date_started: form.value.date_started || null,
      is_active: form.value.is_active,
    })
    if (error) {
      ...
      message.value = { type: 'success', text: 'BOD member added.' }
    }
  }
...
async function toggleActive(m: BodMember) {
  const newStatus = m.is_active === 'Y' ? 'N' : 'Y'
  await supabase.from('tbl_bod_members').update({ is_active: newStatus }).eq('id', m.id)
  await fetchMembers()
}
...
<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-heading font-bold text-navy">BOD Members</h1>
            ...
            <tr v-for="m in filtered" :key="m.id" class="hover:bg-gray-50/50">
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <div
                    class="w-8 h-8 rounded-full bg-navy-100 flex items-center justify-center text-xs font-bold text-navy shrink-0"
                  >
