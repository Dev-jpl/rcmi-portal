...
async function handleSave() {
    ...
    if (editingId.value) {
        const { error } = await supabase.from('tbl_lpath_leaders').update(payload).eq('id', editingId.value)
        message.value = error ? { type: 'error', text: error.message } : { type: 'success', text: 'L-Path leader updated.' }
    } else {
        const { error } = await supabase.from('tbl_lpath_leaders').insert(payload)
        message.value = error ? { type: 'error', text: error.message } : { type: 'success', text: 'L-Path leader added.' }
    }
