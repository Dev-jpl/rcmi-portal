...
const form = ref({
  ...
  network_id: null as number | null,
...
async function fetchNetworkLeaders() {
  const { data } = await supabase
    ...
    .select(
      'id, user_id, pastor_id, pastor_name, user:tbl_users!tbl_network_leaders_user_id_fkey(first_name, last_name)',
    )
    .eq('is_active', 'Y')
    .order('created_at', { ascending: false })
  networkLeaders.value = (data as NetworkOption[]) ?? []
}
...
const filtered = computed(() => {
  ...
  if (networkFilter.value) {
    list = list.filter((m) => m.network_id === networkFilter.value)
  }
...
function openEdit(m: LpathLeader) {
...
const selectedNetworkName = computed(() => {
  if (!form.value.network_id) return null
  const n = networkLeaders.value.find((n) => n.id === form.value.network_id)
  return n ? networkDisplayName(n) : null
})
...
async function handleSave() {
  ...
  const selectedNetwork = networkLeaders.value.find((n) => n.id === form.value.network_id)

  const payload = {
    user_id: form.value.user_id,
    church_id: form.value.church_id,
    church_name: selectedChurch?.church_name ?? null,
    pastor_id: selectedNetwork?.pastor_id ?? null,
    pastor_name: selectedNetwork?.pastor_name ?? null,
  ...
  if (editingId.value) {
    ...
    message.value = error
      ...
      : { type: 'success', text: 'L-Path leader updated.' }
  } else {
    const { error } = await supabase.from('tbl_lpath_leaders').insert(payload)
    message.value = error
      ? { type: 'error', text: error.message }
      : { type: 'success', text: 'L-Path leader added.' }
  }
...
async function toggleActive(m: LpathLeader) {
  const newStatus = m.is_active === 'Y' ? 'N' : 'Y'
  await supabase.from('tbl_lpath_leaders').update({ is_active: newStatus }).eq('id', m.id)
  await fetchLeaders()
}
...
<template>
  <div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-heading font-bold text-navy">L-Path Leaders</h1>
            ...
            <tr v-for="m in filtered" :key="m.id" class="hover:bg-gray-50/50">
              <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                  <div
                    class="w-8 h-8 rounded-full bg-teal-100 flex items-center justify-center text-xs font-bold text-teal-700 shrink-0"
                  >
          ...
          <div class="space-y-4">
            ...
            <div class="relative">
              ...
              >
                ...
                <div
                  v-if="!filteredNetworkOptions.length"
                  class="px-3 py-4 text-center text-xs text-gray-400"
                >
