...
const filtered = computed(() => {
  ...
  return eventStore.events.filter(
    (e) => e.event_title.toLowerCase().includes(q) || e.event_type?.toLowerCase().includes(q),
  )
})
...
async function handleSaveDefault() {
  ...
  try {
    const payload = {
      ...
      event_type: defaultForm.value.event_type || 'General',
      day_of_week: defaultForm.value.day_of_week,
      default_start_time: defaultForm.value.default_start_time || '00:00',
      default_end_time: (defaultForm.value.default_end_time || null) as string | undefined,
    ...
    if (editingDefault.value) {
      const { error: err } = await supabase
        .from('lib_default_events')
        .update(payload)
        .eq('id', editingDefault.value.id)
      if (err) throw err
    } else {
      const { error: err } = await supabase.from('lib_default_events').insert(payload as any)
      if (err) throw err
    }
...
function downloadQR() {
  ...
  a.download = `event-qr-${qrTarget.value.id}.png`
  a.click()
}
  ...
  <div>
      ...
      <div class="inline-flex gap-0.5 bg-gray-100/80 rounded-md p-0.5">
        ...
        >
          ...
          <span
            ...
            :class="activeTab === tab.key ? 'text-navy/50' : 'text-gray-300'"
          >
        ...
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              ...
              stroke-linejoin="round"
              stroke-width="1.5"
              d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zm0 9.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zm9.75-9.75A2.25 2.25 0 0115.75 3.75H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zm0 9.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25a2.25 2.25 0 01-2.25-2.25v-2.25z"
            />
          </svg>
        </button>
        <button
    ...
    <template v-if="activeTab === 'all'">
      ...
      >
        ...
        >
          ...
          <div
            v-else
            class="h-24 bg-gradient-to-br from-navy/5 to-navy/10 flex items-center justify-center"
          >
          ...
          <div class="p-4">
            <div class="flex items-start justify-between gap-3 mb-3">
              <div class="min-w-0 flex-1">
                ...
                <span v-if="evt.event_type" class="text-[11px] text-gray-400">{{
                  evt.event_type
                }}</span>
              </div>
              <button
            ...
            <div class="flex items-center gap-2 pt-3 border-t border-gray-100">
              ...
              <button
                class="text-xs text-gray-500 hover:text-navy font-medium"
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="evt in filtered" :key="evt.id" class="hover:bg-gray-50/50">
                ...
                <td class="px-4 py-3 text-right space-x-2">
                  ...
                  >
                  <button
                    class="text-gray-500 hover:text-navy text-sm font-medium"
                    @click="openEditEvent(evt)"
                  >
                    Edit
                  </button>
                  <button
                    class="text-violet-500 hover:text-violet-700 text-sm font-medium"
                    @click="openEventQr(evt)"
                  >
                    QR
                  </button>
    ...
    <template v-else>
      ...
      <div v-else-if="!defaultItems.length" class="text-center py-16">
        ...
        <button
          @click="openAddDefault"
          class="mt-3 text-sm text-navy font-semibold hover:underline"
        >
          Add your first one
        </button>
      </div>
        ...
        >
          <div class="flex items-start justify-between gap-3 mb-3">
            <div class="min-w-0 flex-1">
              ...
              <span v-if="item.event_type" class="text-[11px] text-gray-400">{{
                item.event_type
              }}</span>
            </div>
            <button
      ...
      <div v-else-if="defaultItems.length" class="space-y-3">
        ...
        >
          <!-- Day badge -->
          <div
            class="w-12 h-12 rounded-lg bg-navy/5 flex flex-col items-center justify-center shrink-0"
          >
            <span class="text-[10px] font-semibold text-navy/50 uppercase leading-none">{{
              item.day_of_week?.slice(0, 3)
            }}</span>
            <span class="text-xs font-bold text-navy mt-0.5">{{
              formatTime(item.default_start_time)
            }}</span>
          </div>
          ...
          <div class="min-w-0 flex-1">
            ...
            <div class="flex items-center gap-2 mt-0.5">
              ...
              <span class="text-[11px] text-gray-400"
                >{{ formatTime(item.default_start_time) }} –
                {{ formatTime(item.default_end_time) }}</span
              >
              <span
                v-if="item.allow_self_checkin"
                class="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-50 text-blue-600 font-medium"
      ...
      >
        <div class="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            ...
            <button class="text-gray-400 hover:text-gray-600 text-xl" @click="modalOpen = false">
              &times;
            </button>
          </div>
          ...
          <form class="px-6 py-5 space-y-4" @submit.prevent="handleSaveEvent">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-navy/20"
                >
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  type="datetime-local"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/20"
                />
              </div>
              <div>
            ...
            <div>
              <label class="flex items-center gap-2 mb-2 cursor-pointer">
                <input
                  v-model="form.specify_checkin_time"
                  type="checkbox"
              ...
              <p v-else class="text-[10px] text-gray-400 pl-6">
                By default, check-in starts at 1:00 AM on the event date.
              </p>
            </div>
            <div>
              ...
              <textarea
                ...
                rows="2"
                class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 resize-none"
              />
            </div>
            <div class="flex flex-col gap-3">
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                <input
                  v-model="form.allow_self_checkin"
                  type="checkbox"
                  class="w-4 h-4 rounded border-gray-300 text-navy focus:ring-navy"
                />
            ...
            <div class="flex justify-end gap-3 pt-2">
              <button
                type="button"
                class="px-4 py-2.5 border border-gray-200 text-gray-500 font-medium rounded-lg hover:bg-gray-50 text-sm"
          ...
          <div class="relative bg-white rounded-lg shadow-2xl max-w-md w-full p-6">
            ...
            <form @submit.prevent="handleSaveDefault" class="space-y-4">
              <div>
                ...
                <input
                  v-model="defaultForm.event_title"
                  type="text"
                  class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy"
              ...
              <div>
                ...
                <select
                  v-model="defaultForm.event_type"
              ...
              <div class="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    v-model="defaultForm.default_start_time"
                    type="time"
                    class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy"
                  />
                </div>
                <div>
                  ...
                  <input
                    v-model="defaultForm.default_end_time"
                    type="time"
                    class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy"
                  />
                </div>
                <div>
                  ...
                  <input
                    v-model="defaultForm.default_checkin_time"
                    type="time"
                    class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy"
                  />
                </div>
              ...
              <div class="flex items-center gap-6">
                <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                  <input
                    v-model="defaultForm.allow_self_checkin"
                    type="checkbox"
      ...
      >
        <div class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 text-center">
          ...
          <p class="text-xs text-gray-400 mb-4">
            Members can scan this QR to check in to this event
          </p>
      ...
      >
        <div class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6">
          ...
          <p class="text-sm text-gray-600 mb-5">
            Are you sure you want to delete <strong>{{ deleteTarget?.event_title }}</strong
            >? All associated attendance logs may be affected.
          </p>
