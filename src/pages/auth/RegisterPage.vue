        ...
        switch (key) {
            ...
            case 'password':
                ...
                if (touched.value.confirmPassword) {
                    if (confirmPassword.value && confirmPassword.value !== password.value) e.confirmPassword = 'Passwords do not match'
...
watch(firstName, () => { if (touched.value.firstName) validate('firstName') })
watch(lastName, () => { if (touched.value.lastName) validate('lastName') })
watch(satelliteChurchId, () => { if (touched.value.church) validate('church') })
watch(email, () => { if (touched.value.email) validate('email') })
watch(password, () => { if (touched.value.password) validate('password') })
watch(confirmPassword, () => { if (touched.value.confirmPassword) validate('confirmPassword') })
...
const step1Valid = computed(() =>
    firstName.value.trim() && lastName.value.trim() && satelliteChurchId.value &&
    !errors.value.firstName && !errors.value.lastName && !errors.value.church
)
...
const friendlyError = computed(() => {
    ...
    if (e.includes('already registered') || e.includes('already been registered') || e.includes('unique')) {
...
async function handleRegister() {
    touched.value = { firstName: true, lastName: true, church: true, email: true, password: true, confirmPassword: true }
    validate()
    ...
    <div>
        ...
        <div class="flex items-center gap-3 mb-7">
            <div class="flex items-center gap-2 flex-1">
                ...
                <span class="text-xs font-medium" :class="step >= 1 ? 'text-navy' : 'text-gray-400'">Personal Info</span>
            </div>
            ...
            <div class="flex items-center gap-2 flex-1">
                ...
                <span class="text-xs font-medium" :class="step >= 2 ? 'text-navy' : 'text-gray-400'">Account</span>
            </div>
        </div>
        ...
        <form @submit.prevent="step === 1 ? goToStep2() : handleRegister()" novalidate>
            ...
            <div v-show="step === 1" class="space-y-4">
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input
                            ...
                            :class="touched.firstName && errors.firstName ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-gray-50/50 hover:border-gray-300'"
                            placeholder="Juan"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            required
                            class="w-full px-3.5 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy transition-all"
                            :class="touched.lastName && errors.lastName ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-gray-50/50 hover:border-gray-300'"
                ...
                <div>
                    <label for="church" class="block text-sm font-medium text-gray-700 mb-1.5">Satellite Church</label>
                    <div class="relative">
                        ...
                        <select
                            ...
                            :class="touched.church && errors.church ? 'border-red-300 bg-red-50/50' : 'border-gray-200 hover:border-gray-300'"
                    ...
                    <p v-if="touched.church && errors.church" class="text-xs text-red-500 mt-1">{{ errors.church }}</p>
            ...
            <div v-show="step === 2" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            :class="touched.email && errors.email ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-gray-50/50 hover:border-gray-300'"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            :class="touched.password && errors.password ? 'border-red-300 bg-red-50/50' : 'border-gray-200 bg-gray-50/50 hover:border-gray-300'"
                    ...
                    <div v-if="password && !errors.password" class="mt-2">
                        <div class="flex items-center gap-2">
                            <div class="flex-1 flex gap-1">
                                <div v-for="i in 4" :key="i" class="h-1 flex-1 rounded-full transition-colors" :class="i <= passwordStrength.level ? passwordStrength.color : 'bg-gray-200'" />
                            </div>
                ...
                <!-- Confirm Password -->
                <div>
                    <label for="reg-confirm" class="block text-sm font-medium text-gray-700 mb-1.5">Confirm Password</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                            <svg class="w-4.5 h-4.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <input
                            ...
                            autocomplete="new-password"
                            class="w-full pl-11 pr-3 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-navy/20 focus:border-navy transition-all"
                            :class="touched.confirmPassword && errors.confirmPassword ? 'border-red-300 bg-red-50/50' : confirmPassword && confirmPassword === password ? 'border-green-300 bg-green-50/30' : 'border-gray-200 bg-gray-50/50 hover:border-gray-300'"
                            placeholder="Re-enter your password"
                            @blur="markTouched('confirmPassword')"
                        />
                    </div>
                    <p v-if="touched.confirmPassword && errors.confirmPassword" class="text-xs text-red-500 mt-1">{{ errors.confirmPassword }}</p>
                    <p v-else-if="confirmPassword && confirmPassword === password" class="text-xs text-green-600 mt-1 flex items-center gap-1">
                ...
                <label class="flex items-start gap-2.5 cursor-pointer pt-1">
                    <input v-model="agreeTerms" type="checkbox" class="w-4 h-4 rounded border-gray-300 text-navy focus:ring-navy mt-0.5" />
                    <span class="text-xs text-gray-500 leading-relaxed">
                        I understand that my account will need to be <strong class="text-gray-700">approved by a church administrator</strong> before I can access the portal.
                    </span>
                </label>
